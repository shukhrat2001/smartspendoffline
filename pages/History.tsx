...
export const History: React.FC<HistoryProps> = ({ onTriggerPremium }) => {
  ...
  const handleExport = () => {
    if (!isPremium) {
      onTriggerPremium("Exporting data to CSV/PDF is a Premium feature.");
      return;
    }
          ...
          {filteredExpenses?.map((expense, index) => {
            ...
            if (isLocked) {
              ...
              return (
                <div 
                  key={expense.id} 
                  onClick={() => onTriggerPremium("Upgrade to view expense history older than 30 days.")}
            ...
            return (
              <div key={expense.id} className="bg-white p-4 rounded-xl border border-gray-100 shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-gray-900">{expense.categoryName}</p>
