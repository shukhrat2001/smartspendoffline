      ...
      <div className="bg-gradient-to-br from-indigo-600 to-purple-700 text-white p-6 pb-12 rounded-b-[2.5rem] shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex justify-between items-start mb-6">
            <div>
              <p className="text-indigo-100 text-sm font-medium mb-1">Total Spent Today</p>
              <h1 className="text-4xl font-bold tracking-tight">{formatCurrency(todayTotal)}</h1>
            </div>
            {!isPremium && (
              <button 
                onClick={() => onTriggerPremium("Upgrade to unlock cloud sync and backup.")}
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-xl p-4 flex items-center justify-between border border-white/10">
             ...
             <div className="flex items-center gap-3">
               ...
               <div onClick={() => onTriggerPremium("Set budgets with Premium.")}>
                 <p className="text-xs text-indigo-200">Budget</p>
                 <p className="font-semibold text-sm">Set Limit</p>
               </div>
             </div>
          </div>
        </div>
      ...
      {!isPremium && (
        <div className="mx-4 -mt-6 mb-6 relative z-20">
          <div className="bg-white p-4 rounded-xl shadow-lg border border-indigo-100 flex items-center justify-between">
            <div>
              <p className="text-indigo-900 font-bold text-sm">Upgrade to Premium</p>
              <p className="text-gray-500 text-xs">Unlock unlimited history & export</p>
            </div>
            <button 
              onClick={() => onTriggerPremium("Unlock all features today.")}
        ...
        <div className="space-y-3">
          {expenses?.length === 0 ? (
            <div className="text-center py-10 bg-white rounded-2xl border border-dashed border-gray-200">
              ...
              <p className="text-gray-400 text-xs mt-1">Tap + to add your first one</p>
            </div>
          ) : (
            expenses?.map((expense) => (
              <div key={expense.id} className="bg-white p-4 rounded-2xl shadow-sm border border-gray-50 flex items-center justify-between active:scale-[0.99] transition-transform">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="font-bold text-gray-900 capitalize">{expense.categoryName}</p>
